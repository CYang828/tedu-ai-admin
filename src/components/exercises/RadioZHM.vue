<template>
  <div class="RadioZHM"  @dblclick="handleDoubleClick(dataListItem)">
    ...
    <div class="options-box">
      <el-radio-group v-model="radioIdx" style="display: block;" :disabled="!isEdit" @change="changeRadio" @click.native.stop>
        <el-radio :class="{'noEdit':!isEdit}" :label="idx"  v-for="(item,idx) in dataListItem.options" @click.native.stop>
          ...
          <el-button v-if="idx === dataListItem.options.length - 1" style="margin-left: 20px;" type="primary" @click.prevent="addOptionsFn" v-show="isEdit">添加选项</el-button>
...
const addOptionsFn = ()=>{
  let dataListItemCopy = JSON.parse(JSON.stringify(props.dataListItem))
  if(dataListItemCopy.options.length == 24){
    ElMessage({
      message: '选项最多24个',
      type: 'warning',
    })
  ...
  dataListItemCopy.options.push({
    "id": dataListItemCopy.options.length+1 + (new Date().getTime() + ''),
    "questionId": null,
    "key": null,
    "name": "",
    "isAnswer": 0
...
onMounted(()=>{
  props.dataListItem.options.find((item,idx)=>{
    if(item.isAnswer === 1){
      radioIdx.value = idx
      return item
    }
